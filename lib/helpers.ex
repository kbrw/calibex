defmodule Calibex.Helper do
  ...
  def fill_rec([{_,_}|_]=props,fill_by_parent,parent) do
    props = Enum.map(props,fn {k,v}->
      v = if fill_by_parent[parent] && k in fill_by_parent[parent] do
        augment(k,v,props)
      else v end
  ...
  def default(:prodid,_vals), do: "-//KBRW//Calibex #{unquote(Mix.Project.config[:version])}//EN"
  def default(_,_), do: ""

  def parent(k) when k in [:uid,:last_modified,:sequence,:dtstamp,:created,:status,:transp,:attendee,:organizer], do: :vevent
